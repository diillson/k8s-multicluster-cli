...
var labelCmd = &cobra.Command{
	Use:   "label RESOURCE NAME KEY=VAL [KEY=VAL]...",
	Short: "Adiciona/altera labels em recursos em múltiplos clusters (kubectl label)",
	Args:  cobra.MinimumNArgs(3),
	Run:   runLabelUniversal,
}

func init() {
	labelCmd.Flags().StringVarP(&labelNamespace, "namespace", "n", "", "Namespace para recursos namespaced")
	rootCmd.AddCommand(labelCmd)
}

func runLabelUniversal(cmd *cobra.Command, args []string) {
	resource := args[0]
	name := args[1]
	labels := args[2:]
	...
	utils.OutputLabel(handler.Results)
}
