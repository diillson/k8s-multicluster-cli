...
func (h *CreateJobHandler) RunOnCluster(ctx context.Context, cluster executor.ClusterInfo, kubeConfigPath string) error {
...
func (h *CreateJobHandler) appendResult(cluster executor.ClusterInfo, name, ns string, success bool, err error) {
	h.mu.Lock()
	defer h.mu.Unlock()
	h.Results = append(h.Results, CreateJobResult{
		Cluster: cluster, Name: name, Namespace: ns, Success: success, Err: err,
	})
}
func (h *CreateJobHandler) Result() interface{} { return h.Results }
